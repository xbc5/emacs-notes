...
It returns the modified HTABLE (for convenience)."
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'note-category (xtag-single 'tv-category "TV category" t) result)
    (xht-default 'genres (xtag-multi 'tv-genre "TV genres" t) result)
    (xht-default 'actors (xtag-multi 'tv-actor "TV actors" t) result)
    (xht-default 'writers (xtag-multi 'tv-writer "TV writers" t) result)
    (xht-default 'directors (xtag-multi 'tv-director "TV directors" t) result)
    ...
    (xht-default 'state (xtag-single 'watched-state "Watched state" t) result)
    ...
    (xht-default 'contexts (xtag-multi 'tv-context "TV contexts" t) result)
    result))

;; TODO: create a generic function that will save all tags.
...
(defun xvulpea--article-meta-defaults (htable)
  ...
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'note-category (xtag-single 'article-category "Article category" t) result)
    (when (xtag-p 'needs-cover (ht-get result 'note-category))
      (xht-default 'view-url (lambda () (xprompt-url "View URL")) result))
    (when (xtag-p 'needs-rating (ht-get result 'note-category))
      (xht-default 'rating (lambda () (xprompt-rating "Your rating")) result))
    (when (xtag-p 'needs-year (ht-get result 'note-category))
      (xht-default 'year (lambda () (xprompt-year "Year")) result))
    (when (xtag-p 'needs-state (ht-get result 'note-category))
      (xht-default 'state (xtag-single 'state "State" t t) result))
    (when (xtag-p 'needs-url (ht-get result 'note-category))
...
(defun xvulpea--song-meta-defaults (htable)
  ...
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'artists (xtag-multi 'music-artist "Music artists" t) result)
    (xht-default 'genres (xtag-multi 'music-genre "Music genres" t) result)
    (xht-default 'contexts (xtag-multi 'music-context "Music contexts" t) result)
    (xht-default 'rating (lambda () (xprompt-rating "Your rating" t)) result)
...
(defun xvulpea--game-meta-defaults (htable)
  ...
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'genres (xtag-multi 'game-genre "Game genres" t) result)
    (xht-default 'publisher (xtag-single 'game-publisher "Game publisher" t) result)
    (xht-default 'developer (xtag-single 'game-developer "Game developer" t) result)
    (xht-default 'view-url (lambda () (xprompt-yt "Trailer URL (YouTube)" t)) result)
    (xht-default 'year (lambda () (xprompt-year "Year published (or expected)" t)) result)
    (xht-derive result 'year 'period (lambda (year) (xvulpea--period year)))
    (cond ((xtime-year-p (ht-get result 'year) '=)
           (ht-set result 'release-state (xtag-single 'release-state "Release state" nil t)))
          ((xtime-year-p (ht-get result 'year) '>)
           (ht-set result 'release-state "unreleased"))
          ((xtime-year-p (ht-get result 'year) '<)
           (ht-set result 'release-state "released")))
    (ht-set result 'state (xtag-single 'game-played-state "Played state"))
...
(defun xvulpea--idea-meta-defaults (htable)
  ...
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'roam-aliases (xprompt-aliases nil t) result)
    (xht-default 'note-category (xtag-single 'idea-type "Type of idea" t) result)
    (when (string= (ht-get result 'note-category) "project")
      (xht-default 'project-type (xtag-single 'project-type "Type of project" t) result))
    result))
...
(defun xvulpea--person-meta-defaults (htable)
  ...
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'roam-aliases (xprompt-aliases nil t) result)
    (xht-default 'note-category (xtag-single 'person-type "Type of person" t) result)
    result))
...
(defun xvulpea--quote-meta-defaults (htable)
  ...
  (let* ((result (ht-copy htable)))
    ...
    (xht-default 'roam-aliases (xprompt-aliases nil t) result)
    (xht-default 'note-category (xtag-single 'quote-type "Type of quote" t) result)
